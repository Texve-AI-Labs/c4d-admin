...
const getVehiclesFromAccount = (account) => {
  ...
  const list = [
    ...(Array.isArray(account?.autos) ? account.autos : []),
  ];
  const seen = new Set();
...
const extractCabIdsFromAccount = (account) => {
  return getVehiclesFromAccount(account)
    .map((item) => item?.id)
...
const CompletedOnboardingDetails = () => {
  ...
  const fetchOnboardingDetails = async () => {
    try {
      ...
      const serviceType = normalizeServiceType(base?.type);
      ...
      try {
        ...
        if (matched && Array.isArray(matched?.autos)) {
          const strictCabs = matched.autos.filter(
            (auto) =>
              auto &&
              auto.id !== null &&
              auto.id !== undefined &&
              String(auto?.accountId ?? auto?.AccountId) === String(id)
          );
  ...
  const refreshCabDetails = async (requestIds = cabRequestIds, options = {}) => {
    ...
    const responses = await Promise.allSettled(
      requestIds.map((itemId) => ApiRequestUtils.get(API_ROUTES.GET_AUTO_BY_ID + `${itemId}`))
    );
  ...
  const parseAddress = (address) => {
    ...
    let cleanedParts = pincodeMatch
      ? parts.map((part) => part.replace(pincodeMatch[0], "").trim()).filter(Boolean)
      : parts;

    // Drop country token so state does not become "India"
    const lastPart = String(cleanedParts[cleanedParts.length - 1] || "").toLowerCase();
    if (lastPart === "india") {
      cleanedParts = cleanedParts.slice(0, -1);
    }
  ...
  const handleVehicleStatusUpdate = async (sectionId) => {
    ...
    try {
      ...
      const res = await ApiRequestUtils.update(API_ROUTES.UPDATE_AUTO_DETAILS, autoDetails);
