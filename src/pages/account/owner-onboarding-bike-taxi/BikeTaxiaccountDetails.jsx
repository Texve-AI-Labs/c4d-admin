...
const BikeTaxiAccountOnboardingDetails = () => {
  ...
  const isAddressFormComplete = useMemo(() => {
    ...
    return (
      ...
      district.length >= 2 &&
      String(addressForm.accountDistrict || "").trim().length > 0 &&
      state.length > 0 &&
      STATE_LIST.some((item) => item.value === state || item.label === state) &&
      /^\d{6}$/.test(pincode)
    );
  }, [addressForm.address, addressForm.street, addressForm.thaluk, addressForm.district, addressForm.accountDistrict, addressForm.state, addressForm.pincode]);
  const missingAddressFields = useMemo(() => {
    ...
    if (!String(addressForm.accountDistrict || "").trim()) missing.push("Account District");
  ...
  const handleContinue = () => {
    ...
    navigate(`/dashboard/vendors/account/owner-onboarding-bike-taxi/details/vehicle/${id}`);
  };
    ...
    <div className="p-4 bg-white rounded-lg shadow-md">
      ...
      <div className="mb-4">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
          <div>
            <Typography variant="h6" color="blue-gray">Bike Taxi Account Document Details</Typography>
            <Typography className="text-sm text-blue-gray-700 mt-1">Bike ID: {id}</Typography>
          </div>          
        </div>
...
export default BikeTaxiAccountOnboardingDetails;
