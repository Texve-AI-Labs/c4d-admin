...
const DRIVER_FEEDBACK_SCHEMA = Yup.object().shape({
  ...
  selectedZones: Yup.array().of(Yup.string().trim()).min(1, "At least one zone is required"),
});
...
const DriverFeedbackEdit = () => {
  ...
  useEffect(() => {
    if (location.state?.gst) {
      ...
      setData({
        ...
        selectedZones: Array.isArray(raw.config?.zones)
          ? raw.config.zones
  ...
  const hasServiceAreas = useMemo(() => serviceAreaOptions.length > 0, [serviceAreaOptions]);
  const zoneOptions = useMemo(
    () => serviceAreaOptions.map((zoneName) => ({ label: zoneName, value: zoneName })),
    [serviceAreaOptions]
  );
  ...
  const onSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      const zones = (values.selectedZones || [])
        .map((zone) => String(zone).trim())
        .filter(Boolean)
        .map((zone) => ({ zone, enabled: true }));
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                ...
                <Select
                  isMulti
                  options={zoneOptions}
                  value={zoneOptions.filter((option) => values.selectedZones.includes(option.value))}
                  onChange={(selected) => {
                    const nextValues = Array.isArray(selected) ? selected.map((item) => item.value) : [];
                    setFieldValue("selectedZones", nextValues);
                  }}
                  placeholder="Select Zones"
                  isDisabled={!hasServiceAreas}
                  className="w-full"
                  name="selectedZones"
                />
                <p className="text-xs text-gray-500 mt-1">Select one or more zones.</p>
                <ErrorMessage name="selectedZones" component="div" className="text-red-500 text-sm" />
              </div>
              <div className="col-span-2">
