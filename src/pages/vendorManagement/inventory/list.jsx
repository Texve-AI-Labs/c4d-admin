...
export const VendorManagementInventory = () => {
      ...
      <CardBody className="pt-0 px-0 pb-2">
        <div className="overflow-x-auto">
          ...
          <table className="w-full min-w-[1100px] table-auto">
            <thead>
              <tr>
                {["Vendor ID", "Product ID", "Available", "Reserved", "Status", "Actions"].map((h) => (
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                rows.map((row) => (
                  <tr key={row?.inventoryId ?? row?.id ?? JSON.stringify(row)}>
                    ...
                    <td className="border-b border-blue-gray-50 py-3 px-5 text-sm">{row?.availableQty ?? "-"}</td>
                    <td className="border-b border-blue-gray-50 py-3 px-5 text-sm">{row?.reservedQty ?? "-"}</td>
                    <td className="border-b border-blue-gray-50 py-3 px-5 text-sm">{row?.status ? "Active" : "Inactive"}</td>
                    <td className="border-b border-blue-gray-50 py-3 px-5">
                      <div className="flex gap-2">
                        <Button size="sm" variant="outlined" onClick={() => navigate(`/dashboard/vendor-management/inventory/details/${row?.inventoryId ?? row?.id}`)}>
                          Details
                        </Button>
                        <Button size="sm" className={ColorStyles.bgColor} onClick={() => navigate(`/dashboard/vendor-management/inventory/edit/${row?.inventoryId ?? row?.id}`)}>
                          Edit
                        </Button>
                      </div>
                    </td>
