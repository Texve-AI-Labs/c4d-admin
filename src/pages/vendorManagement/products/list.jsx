...
export const VendorManagementProducts = () => {
    ...
    <Card className="mx-auto mt-8 w-full">
      <CardHeader variant="gradient" className={`mb-4 rounded-xl p-6 ${ColorStyles.bgColor}`}>
        <div className="flex items-center justify-between gap-4">
          ...
          <Button className="bg-white text-black font-bold hover:bg-gray-100" onClick={() => navigate("/dashboard/vendor-management/products/add")}>
            Add New
          </Button>
        </div>
      </CardHeader>
      <CardBody className="pt-0 px-0 pb-2">
        <div className="overflow-x-auto">
          ...
          <table className="w-full min-w-[900px] table-auto">
            <thead>
              <tr>
                {["Name", "SKU", "Brand", "Status", "Actions"].map((h) => (
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                rows.map((row) => (
                  <tr key={row?.productId ?? row?.id ?? JSON.stringify(row)}>
                    <td className="border-b border-blue-gray-50 py-3 px-5 text-sm">{row?.name ?? "-"}</td>
                    <td className="border-b border-blue-gray-50 py-3 px-5 text-sm">{row?.sku ?? "-"}</td>
                    <td className="border-b border-blue-gray-50 py-3 px-5 text-sm">{row?.brand ?? "-"}</td>
                    <td className="border-b border-blue-gray-50 py-3 px-5 text-sm">{row?.status ? "Active" : "Inactive"}</td>
                    <td className="border-b border-blue-gray-50 py-3 px-5">
                      <div className="flex gap-2">
                        <Button size="sm" variant="outlined" onClick={() => navigate(`/dashboard/vendor-management/products/details/${row?.productId ?? row?.id}`)}>
                          Details
                        </Button>
                        <Button size="sm" className={ColorStyles.bgColor} onClick={() => navigate(`/dashboard/vendor-management/products/edit/${row?.productId ?? row?.id}`)}>
                          Edit
                        </Button>
                        {canDelete && (
                          <Button size="sm" color="red" variant="outlined" onClick={() => handleDelete(row?.productId ?? row?.id)}>
                            Delete
                          </Button>
                        )}
                      </div>
