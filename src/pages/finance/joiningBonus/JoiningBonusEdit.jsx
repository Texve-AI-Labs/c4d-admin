...
export default function JoiningBonusEdit() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const [detailResponse, zoneResponse] = await Promise.all([
          ApiRequestUtils.getWithQueryParam(API_ROUTES.MASTER_JOINS_BOUNCE, {}),
          ApiRequestUtils.getWithQueryParam(API_ROUTES.GEO_MARKINGS, { type: "Service Area" }),
        ]);
        const rows = Array.isArray(detailResponse?.data)
          ...
          : Array.isArray(detailResponse?.result)
            ...
            : [];
        const row = rows.find((item) => String(item.id) === String(planId)) || rows[0];
        ...
        if (row) {
          setFormValues({
            ...
            zone: row.zone || "",
            earningStrategy: row.earningStrategy || "",
            validityDays: row.validityDays ?? "",
            earningWindowDays: row.earningWindowDays ?? "",
            price: row.price ?? "",
            status: row.status || "ACTIVE",
          });
        }
      } catch (error) {
        console.error("Error loading joining bonus:", error);
        setAlert({ message: "Failed to load Joining Bonus details." });
      } finally {
        setLoading(false);
      }
    };
  ...
  const onSubmit = async (values) => {
    ...
    try {
      const response = await ApiRequestUtils.update(API_ROUTES.UPDATE_MASTER_JOINS_BOUNCE, payload);
      if (response?.success) {
        navigate("/dashboard/finance/joining-bonus");
      } else {
        setAlert({ message: response?.message || "Failed to update Joining Bonus." });
      }
    } catch (error) {
      setAlert({ message: error?.response?.data?.message || "Failed to update Joining Bonus." });
    }
  };

  if (loading) return <div className="p-6 bg-white rounded-xl">Loading Joining Bonus details...</div>;
  ...
  return (
    <div className="p-4 bg-white rounded-xl sm:p-6 lg:p-8">
      ...
      <h2 className="mb-4 text-2xl font-bold">Edit Joining Bonus</h2>
      <Formik enableReinitialize initialValues={formValues} 
          ...
          <Form>
            <div className="grid grid-cols-1 gap-4 rounded-lg border border-gray-200 p-4 md:grid-cols-2">
              <div>
                ...
                <Field as="select" name="serviceType" className="w-full rounded-md border border-gray-300 p-2">
              ...
              <div>
                ...
                <Field as="select" name="zone" className="w-full rounded-md border border-gray-300 p-2">
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700">Validity Month</label>
