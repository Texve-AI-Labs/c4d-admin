...
const ParcelDailySlotsAdd = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await ApiRequestUtils.post(API_ROUTES.PROCESS_PARCEL_DAILY_SLOTS, { date });

      if (!response?.success) {
        throw new Error(response?.message || "Unable to add parcel daily slot");
      }

      navigate("/dashboard/finance/parcel-daily-slots");
    } catch (submitError) {
      console.error("Failed to add parcel daily slot:", submitError);
      setError(submitError?.message || "Failed to add parcel daily slot");
    } finally {
      setLoading(false);
    }
  };
          ...
          <form className="grid grid-cols-1 gap-5 md:grid-cols-2" onSubmit={handleSubmit}>
            ...
            <div className="flex flex-col-reverse gap-3 md:col-span-2 md:flex-row md:justify-center">
              <Button
                ...
                onClick={() => navigate("/dashboard/finance/parcel-daily-slots")}
                disabled={loading}
              >
                Back
              </Button>
...
export default ParcelDailySlotsAdd;
