...
const normalizeCategoryPricings = (priceData = {}) => {
    if (Array.isArray(priceData.categoryPricings) && priceData.categoryPricings.length > 0) {
        return priceData.categoryPricings.map((item) => ({
            ...
            carTypes: parseMaybeJson(item.carTypes, []).map(normalizeCarType),
    ...
    if (priceData.category || priceData.pricing) {
        return [{
            ...
            carTypes: parseMaybeJson(priceData.carTypes, []).map(normalizeCarType),
            pricing: normalizePricing(parseMaybeJson(priceData.pricing, {})),
        }];
    }

    return [cloneBikeCategoryPricing()];
};
...
const buildBikePackagePayload = (values, packageId) => {
    return {
...
const BikeMasterPriceTableEdit = () => {
    ...
    const fetchPriceDetails = async (packageId) => {
        try {
            ...
            if (data?.success) {
                ...
                const values = {
                    serviceType: priceData.serviceType || 'BIKE',
            ...
            console.error('Error fetching bike price details:', error);
        }
    };
    ...
    const onSubmit = async (values) => {
        try {
            const reqBody = buildBikePackagePayload(values, id);

            const response = await ApiRequestUtils.post(API_ROUTES.BIKE_PRICE_EDIT, reqBody);
            if (response?.success) {
                navigate('/dashboard/finance/master-price');
            }
        } catch (error) {
            console.error('Error updating bike price details:', error?.response?.data || error);
        }
    };

    return (
        <div className="p-4 mx-auto bg-white">
            <h2 className="text-2xl font-bold mb-4">Bike Pricing Edit</h2>
...
export default BikeMasterPriceTableEdit;
