...
import { Button, Card, CardBody, CardHeader, Input, Spinner, Switch, Typography } from "@material-tailwind/react";
import { useLocation, useNavigate, useParams } from "react-router-dom";
import { ApiRequestUtils } from "@/utils/apiRequestUtils";
import { API_ROUTES, ColorStyles } from "@/utils/constants";
import { buildPayload, BONUS_TYPE_OPTIONS, SERVICE_TYPE_OPTIONS, toForm, validationSchema } from "./utils";

const ErrorMessage = ({ error }) => (error ? <p className="mt-1 text-sm text-red-500">{error}</p> : null);

export default function DriverRadiusBonusEdit() {
  const { id } = useParams();
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const updateRoute = `${API_ROUTES.DRIVER_DISPATCH_ZONE_CONFIGS}/${id}`;
    ...
    <div className="mb-8 mt-8">
      <Card>
        <CardHeader variant="gradient" className={`mb-4 rounded-xl p-6 ${ColorStyles.bgColor}`}>
          <Typography variant="h6" color="white">Edit Driver Radius & Bonus Configuration</Typography>
        </CardHeader>
        <CardBody>
